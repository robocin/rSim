...
struct VSS {
    VSS(int fieldType, int nRobotsBlue, int nRobotsYellow, int timeStep_ms,
        const vd& ballPos, const vvd& blueRobotsPos, const vvd& yellowRobotsPos) : m_fieldType(fieldType), m_nRobotsBlue(nRobotsBlue), m_nRobotsYellow(nRobotsYellow), m_timeStep_ms(timeStep_ms){
    ...
    void reset(const vd& ballPos, const vvd& blueRobotsPos, const vvd& yellowRobotsPos) {delete m_world;  m_world = new VSSWorld(m_fieldType, m_nRobotsBlue, m_nRobotsYellow, m_timeStep_ms / 1000.0,
                                                                                                                                 ballPos, blueRobotsPos, yellowRobotsPos);}
...
struct SSL {
    ...
    void step(vvd actions) const { m_world->step(std::move(actions)); }
    vd getState() const { return m_world->getState(); }
    void reset(const vd& ballPos, const vvd& blueRobotsPos, const vvd& yellowRobotsPos) {delete m_world;  m_world = new SSLWorld(m_fieldType, m_nRobotsBlue, m_nRobotsYellow, m_timeStep_ms / 1000.0,
                                                                                                                                 ballPos, blueRobotsPos, yellowRobotsPos);}
    std::unordered_map<std::string, double> getFieldParams() const {return m_world->getFieldParams();}
